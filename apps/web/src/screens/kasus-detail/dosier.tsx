  ...
  return (
    ...
    >
      <header className="border-b border-rule px-3 py-2">
        <h2 id="dosier-heading" className="text-sm font-medium uppercase tracking-wide text-muted">
          Dosier
        </h2>
        <input
          value={query}
          ...
          className="mt-2 w-full rounded-md border border-rule bg-paper px-2 py-1 text-sm"
        />
      </header>
      <ul className="flex-1 overflow-y-auto" role="listbox" aria-label="Daftar entitas">
        {loading ? (
          <li className="px-3 py-2 text-sm text-muted">Memuat…</li>
        ) : matched.length === 0 ? (
          ...
          matched.map((entity) => (
            <li key={entity.id}>
              ...
              >
                ...
                <span className="text-xs text-muted">{NODE_TYPE_LABELS_BAHASA[entity.type]}</span>
              </button>
            </li>
          ))
        )}
