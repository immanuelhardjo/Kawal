# Capability: OSINT Ingestion

## Purpose

Provides request-time, per-user source ingestion from Tier-1 and Tier-2 publishers into the user's dossier. Covers the adapter registry, the SSE progress-streaming protocol, rate limiting, Wayback archive capture, excerpt storage, write-back invariants, per-adapter isolation, and per-user activity logging. No scheduled background workers exist.
