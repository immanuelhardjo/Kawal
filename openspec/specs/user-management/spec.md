...
Defines authentication (Google SSO and email/password), session lifecycle, per-user dossier isolation, account deletion with full cascade, dossier export, and the authentication/write audit log. No roles exist beyond being an authenticated owner of one's own dossier.
