...
Provides a deterministic, idempotent seed script (`pnpm seed:dev`) that populates a local development database with a rich, realistic set of fixture data covering all UI-visible aggregate types. This enables developers and testers to start from a consistent, fully-populated state without manually creating data.
