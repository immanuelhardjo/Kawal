# Capability: Entity Dossier

## Purpose

Models the four entity types (person, institution, company, document) within a user's dossier and defines the profile contents rendered for each. Entities are scoped to one owner, may be linked across cases within that owner's dossier, and carry a right-of-reply slot and append-only revision history.
