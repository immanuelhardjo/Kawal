...
#### Scenario: Live re-render on timeline scrub

- **WHEN** the user drags the timeline cursor and the `asOfDate` updates
...
The system SHALL open the entity's dossier profile in the Kasus Detail Profil panel when a graph node is tapped, and SHALL open the source side panel showing the relationship's verbatim source excerpt and source URL when an edge is tapped. Tapping a node SHALL also filter the Garis Waktu section to events involving that entity, and SHALL highlight that entity in the Dosier section.

#### Scenario: Tap a node

- **WHEN** the user taps an entity node in Peta Kasus
- **THEN** the Profil panel for that entity (scoped to the current case) opens
- **AND** the Garis Waktu filter is set to events involving that entity
- **AND** the Dosier section highlights that entity

#### Scenario: Tap an edge

- **WHEN** the user taps a relationship edge
- **THEN** a side panel opens showing the source excerpt verbatim, the source URL, and the archive URL
