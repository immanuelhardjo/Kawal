...
- [x] 1.1 Add `@fontsource/playfair-display`, `@fontsource/ibm-plex-serif`, and `@fontsource/special-elite` to `apps/frontend/package.json` and install via pnpm
- [x] 1.2 Remove any Google Fonts `<link>` tags from `apps/frontend/index.html`

## 2. Tailwind Token System

- [x] 2.1 Replace the `colors` block in `apps/frontend/tailwind.config.js` with the new token map: `board`, `board-shadow`, `parchment`, `chalk`, `chalk-muted`, `string`, `amber-pin`, `rule`, and all `stamp-*` tokens with exact hex values from the visual-identity spec
- [x] 2.2 Update `fontFamily` in `apps/frontend/tailwind.config.js`: `sans` → IBM Plex Serif, add `headline` → Playfair Display, `mono` → Special Elite

## 3. Global CSS

- [x] 3.1 Add Fontsource imports to `apps/frontend/src/styles/index.css` (`@fontsource/playfair-display`, `@fontsource/ibm-plex-serif`, `@fontsource/special-elite`)
...
## 4. Component Class Rename (apps/frontend/src/screens)

- [x] 4.1 Replace `bg-paper` → `bg-board` across all files in `apps/frontend/src/screens/**`
- [x] 4.2 Replace `text-ink` → `text-chalk` across all files in `apps/frontend/src/screens/**`
- [x] 4.3 Replace `text-muted` → `text-chalk-muted` across all files in `apps/frontend/src/screens/**`
...
- [x] 4.7 Run `grep -r "bg-paper\|text-ink\|text-muted\|border-rule\|font-serif\|bg-accent" apps/frontend/src` to confirm zero remaining occurrences of old tokens
...
- [x] 6.1 Update `<Background>` in `apps/frontend/src/screens/kasus-detail/peta-kasus.tsx` to use `color="#2D5040"` (chalk-green dots)
...
- [x] 8.1 Run `pnpm build` in `apps/frontend` — confirm zero Tailwind class errors and clean build output
